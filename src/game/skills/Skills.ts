...
export interface Skill {
  ...
  execute: (player: Player, direction?: THREE.Vector3, getEnemies?: () => any[]) => void; // スキル実行関数
}

// スキルの実装を集約するクラス
export class Skills {
  // 魔法のオーブ (旧basicAttack)
  static magicOrb(player: Player, direction?: THREE.Vector3, getEnemies?: () => any[]): void {
    // 魔法のオーブエフェクトを表示
    Skills.createMagicOrbEffect(player, direction, getEnemies);
  }
  
  // 魔法のオーブ攻撃エフェクトを表示するメソッド
  static createMagicOrbEffect(player: Player, customDirection?: THREE.Vector3, getEnemies?: () => any[]): void {
    // 攻撃エフェクト（球）を作成
    ...
    const direction = customDirection ? customDirection.clone().normalize() : player.direction.clone().normalize();
    ...
    const animate = (timestamp: number) => {
      ...
      if (!isReturning) {
        ...
        if (getEnemies) {
            ...
            for (const enemy of enemies) {
              ...
              if (collision) {
                ...
                try {
                  ...
                  console.log(`Applied ${player.attackPower} damage to enemy. Enemy health: ${enemy.health}/${enemy.maxHealth}`);
          ...
          console.warn('getEnemies function was not provided to createMagicOrbEffect');
        }
        ...
        if (distance > 0) {
          ...
          if (getEnemies) {
            ...
            for (const enemy of enemies) {
              ...
              if (collision) {
                console.log(`${enemy.mesh.name || 'Enemy'} takes ${player.attackPower} damage (return)`);
                enemy.takeDamage(player.attackPower);
                if (typeof enemy.updateHPBar === 'function') {
                  enemy.updateHPBar();
                }
